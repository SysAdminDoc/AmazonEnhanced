...
- Rewrote the README around installation and everyday use, with an illustrated guide and explicit limits for local price history and review heuristics.
- Clarified seller-name, brand-filter and ingredient-watchlist labels without changing their rules.
- Kept release JavaScript and CSS readable.
- Included the project license in all browser packages and made signing fail if the original key is missing, instead of silently changing extension identity.
- Fixed overlapping price-history headings and undefined colors inherited from the popup. The panel follows the saved theme and labels observations without an unsupported currency symbol.
- Made browser verification check the installed extension's identity before inspecting its worker, avoiding a collision with Chromium's built-in background.js worker.
- Updated the isolated Firefox test session for current geckodriver's explicit browser-context access requirement.
- Preserved the original source and artwork for this pass under assets/concepts/2026-09-09-marketing.
...
- **Extra "Sort by" options.** injects *Most reviews*, *Newest*, *Best $/unit* into the search sort dropdown (client-side DOM reorder; uses the existing unit-price calculation).
